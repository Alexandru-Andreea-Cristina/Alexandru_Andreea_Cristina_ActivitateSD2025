//stiva este lista simpla
//coada inserarea se face la un capat- FIFO, first in, first out
//stiva- LIFO - last in, last out
//coada- inserarea- enqueue, extragerea- dequ..
//coada- extragerea-push, inserarea-pop
...
//daca avem poisibilitatea sa alegem, sa alegem LISTA
...
struct Nod {
	Masina info;
	Nod* next; //lista simplu inlantuita
};
...
void pushStack(Nod** stiva, Masina masina) {  //** -> vom mod primul elem al stivei noastre
	Nod* nou = (Nod*)malloc(sizeof(Nod));  // ce retine nmk, ii alocam spatiu
	nou->info = masina;
	nou->next = (*stiva);
	(*stiva) = nou;
}
...
Masina popStack(Nod** stiva) {
	if ((*stiva) != NULL) {
		...
		Nod* prev = (*stiva);
		(*stiva) = (*stiva)->next; //trebuie sa dezalocam
		free(prev);
		return nou;
	}
	Masina m;
...
unsigned char emptyStack(Nod* stiva) {  ///daca stiva resp este goala sau nu
	return stiva == NULL;
}
...
void dezalocareStivaDeMasini(Nod** stiva) {
	while ((*stiva) != NULL) {
		...
		fre(m.numeSofer);
		free(m.model); //deplasarea stivei NU SE FACE CU NEXT, exemplu cu cana, nu poti lua ultima cana daca nu le ridici pe cele dinaintea ei
	}
}
