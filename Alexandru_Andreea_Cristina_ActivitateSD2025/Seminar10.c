...
void rotireStanga(Nod** radacina) {  //facem functia de rotire la stanga, nod**-> se va modifica radacina si void ca nu returnam nmk
	Nod* nod; //salvam radacina de dreapta dar suntem siguri ca exista? da, noi daca apelam rotirea la st avem dezechilibru la dr
	nod = (*radacina)->dr;
	(*radacina)->dr = nod->st; //la fel ca metoda paharelor, st deja e salvat acum
	nod->st = *radacina; //continaum cu met paharelor
	*radacina = nod;
}

//rotirea la dre4apta
void rotireDreapta(Nod** radacina) {
	Nod* aux; //dez in partea st
	aux = (*radacina)->st;
	(*radacina)->st = aux->dr;
	aux->dr = (*radacina);
	*radacina = aux;
...
//facem functia de calcul al echilibrului
...
void adaugaMasinaInArboreEchilibrat(Nod** radacina, Masina masinaNoua) {
	if ((*radacina) != NULL) {
		...
		} //se va face apelul fie pe st, fie pe dr
...
int main() {
	...
	printf("Pretul total al masinilor: %2.f", calculeazaPretTotal);
