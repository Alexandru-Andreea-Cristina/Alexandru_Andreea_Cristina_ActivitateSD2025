//ce e stack?
//arbori binari de cautare - la cautarea unor informatii
//nodul din stanga trebuie sa fie mai mic , iar in dreapta mai mare
//se indentifica prin adresa acestui nod, pointerul din st si dr si INFO
//divide impera- recursivitatea, comparam
// //daca e mai mic in partea st. mai amre in dr
//INSERARE- sa inseram un nou nod, ce trebuie sa facem- comparam
//nod frunza- nu are descendenti, TOT TIMPUL VA FI INSERAT ASA
//nu inseram dublicate in arborele mele SAU prin suprascriere SAU il adaugam ca atare in st sau dr
...
void adaugaMasinaInArbore(Nod** radacina, Masina masinaNoua) {
	if ((*radacina) = !NULL) {
		...
		strcpy_s((*radacina)->info.model, strlen(masinaNoua.model) + 1, masinaNoua.model);
		(*radacina)->info.numeSofer = (char*)malloc(strlen(masinaNoua.numeSofer) + 1);
		strcpy_s((*radacina)->info.numeSofer, strlen(masinaNoua.numeSofer) + 1, masinaNoua.numeSofer);   //inserarea in arbore
	}
}
...
void afisareMasiniIntOrdine(Nod* radacina) {
	if (radacina != NULL) {
		afisareMasiniIntOrdine(radacina->st);
		afisareMasina(radacina->info);
		afisareMasiniIntOrdine(radacina->dr);
	}
}


void afisareaArborePreOrdine(Nod* radacina) {   //intordine- SRD, post- SDR, pre- RSD
	if (radacina != NULL) {
		afisareMasina(radacina->info);
		afisareaArborePreOrdine(radacina->st);
		afisareaArborePreOrdine(radacina->dr);
...
int main() {
	...
	//afisareaArborePreOrdine(arbore);
	afisareMasiniIntOrdine(arbore);
